...
class TemporalGridEncoder(nn.Module):
    ...
    def init_parameters(self) -> None:
        ...
        row, mask_a, mask_b = to_sampling_index(
            index_list[0], permute_list[0], last_entry
        )
        sampling_index = [row]
        ...
        for _ in range(1, temporal_grid_rows - 1):
            ...
            row, mask_a, mask_b = to_sampling_index(
                index_list[-1], permute_list[-1], last_entry
            )
    ...
    def get_temporal_index(
        self, time: Float[Tensor, "bs"]
    ) -> Float[Tensor, "bs temporal_index_dim"]:
    ...
    def get_temporal_tv_loss(self) -> Float[Tensor, ""]:
        ...
        return (
            (self.embeddings[:, feat_idx[0]] - self.embeddings[:, feat_idx[1]])
            .abs()
            .mean()
        )
