...
@dataclass
class VanillaDnerfModelConfig(ModelConfig):
    """VanillaDnerf Model Config"""
    ...
    lipschitz_loss_mult: float = 3e-6
...
class NeRFModel(Model):
    ...
    config: VanillaDnerfModelConfig
    ...
    def get_loss_dict(
        ...
        if self.temporal_distortion.lipschitz is True:
            loss_dict["lipschitz_loss"] = (
                self.config.lipschitz_loss_mult
                * self.temporal_distortion.mlp_deform.get_lipschitz_loss()
            )
        return loss_dict
